"""Google Meet Integration Provider — create & manage video meetings via OAuth.

Google Meet does not have a standalone API; meetings are created through the
Google Calendar API by setting ``conferenceData.conferenceType = "new"``.
"""
    ...
    async def _get_token(self, user_id: UUID | None = None) -> OAuthToken | None:
        ...
        async with AsyncSessionLocal() as session:
            ...
            token = await repo.find_by_user_and_service(
                user_id=user_id, provider="google", service="meet"
            )
            if token is None:
                return None
    ...
    async def health_check(self, user_id: UUID | None = None) -> IntegrationHealth:
        ...
        if token is None:
            return IntegrationHealth(
                ...
                status="disconnected",
                message="No Meet token — connect via OAuth"
            )
        try:
            # Meet health == Calendar API access (Meet is part of Calendar)
            ok = await self._oauth.check_calendar_access(token.access_token)
            if ok:
                return IntegrationHealth(
                    service=self.service_name, connected=True,
                    status="connected", message="Google Meet accessible"
                )
            return IntegrationHealth(
                service=self.service_name, connected=False,
                status="error", message="Meet/Calendar API returned error"
            )
        ...
        try:
            ...
            if token is None:
                return IntegrationAction(
                    ...
                    error="Not connected to Google Meet"
                )
            ...
            if action == "create_meeting":
                ...
                result = await self._oauth.create_calendar_event(
                    ...
                    end_time=end_time,
                    attendees=attendees,
                    conference=True,  # enables Google Meet
                )
                return IntegrationAction(
